...
llama_kv_cache::llama_kv_cache(
    ...
    // initialize per-cell generation counters for prefix cache invalidation
    cell_generations.resize(n_stream);
    for (uint32_t s = 0; s < n_stream; ++s) {
        cell_generations[s].resize(kv_size, 1); // start at 1; 0 means "invalid"
    }
}
...
void llama_kv_cache::prefix_cache_enable(const llama_radix_tree_options & options) {
    prefix_tree = std::make_unique<llama_radix_tree>(options);
...
void llama_kv_cache::prefix_cache_promote(
    ...
    if (token_ids.empty() || token_ids.size() != cell_indices.size()) {
        return;
    }
...
int32_t llama_kv_cache::prefix_cache_find(
    ...
    if (!result.success || result.matched_length == 0) {
        return 0;
    }

    // Validate each matched cell: check that the generation counter hasn't changed
    // since the entry was inserted into the tree.
    ...
    for (int32_t i = 0; i < result.matched_length; ++i) {
        ...
        // generation 0 means already invalidated
        if (gen == 0) {
            break;
        }
        ...
        if (idx < cell_generations[0].size() && cell_generations[0][idx] == gen) {
            ...
            // cell was overwritten - stop here, prefix is broken
            break;
        }
    }
...
void llama_kv_cache::apply_ubatch(const slot_info & sinfo, const llama_ubatch & ubatch) {
    ...
    for (uint32_t s = 0; s < sinfo.n_stream(); ++s) {
        for (uint32_t ii = 0; ii < sinfo.size(); ++ii) {
            ...
            // prefix cache: bump generation counter and invalidate stale tree entries
            {
                const uint32_t strm = sinfo.strm[s];
                if (strm < cell_generations.size() && idx < cell_generations[strm].size()) {
